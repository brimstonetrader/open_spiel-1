...
class _InfoStateNode(object):
  ...
  cfv_accumulator = collections.defaultdict(float)
...
def _update_current_policy(current_policy, info_state_nodes):
  ...
  for info_state, info_state_node in info_state_nodes.items():
    ...
    if info_state_node.update_this_iter:
...
class _CFRSolverBase(object):
  ...
  def _initialize_info_state_nodes(self, state, reach_prob=1.0):
    ...
    if info_state_node is None:
      ...
      info_state_node.reach_prob = reach_prob
      self._info_state_nodes[info_state] = info_state_node

    for action in info_state_node.legal_actions:
      self._initialize_info_state_nodes(state.child(action), reach_prob/len(info_state_node.legal_actions))
  ...
  def _compute_counterfactual_regret_for_player(self, state, policies,
    ...
    info_state_node.reach_prob_sum += reach_probabilities[current_player]
    if info_state_node.reach_prob_sum < 1.0: return state_value
    else: info_state_node.update_this_iter = True
    if policies is None:
