...
def make_opp3(game, agent, opp_player, seed):
    ...
    Works with any agent exposing get_policy(), getdist(), or solver.
    Uses exact_agent_ev (already imported) which returns a callable.
    """
    rng         = random.Random(seed)
...
def make_agent_step_rwywe(game):
    """
    Adaptor for original rwywe.py agents.
    agent.step() may return an int or (if gto.step() misbehaves) something
    else — we coerce to int and fall back to the first legal action if needed.
    """
    def step(agent, state):
        raw_action = agent.step(state)

        # Coerce to int — gto.step() can return a string ISS or other type
        ...
        # _pending_action_probs is set by agent.step() before returning
        pending = agent._pending_action_probs or {}
        norm    = sum(pending.get(a, 0.0) for a in legal) or 1.0
        iss     = state.information_state_string(agent.player_id)

        # Build full policy: pinned at played ISS, uniform elsewhere
        agent_policy = {}
        def walk(s):
            if s.is_terminal(): return
            if s.current_player() == pyspiel.PlayerId.CHANCE:
                ...
                if cur_iss not in agent_policy:
                    ...
                    if cur_iss == iss:
                        agent_policy[cur_iss] = {
                            a: pending.get(a, 0.0) / norm for a in lgl
                        }
                    else:
                        agent_policy[cur_iss] = {a: 1.0 / len(lgl) for a in lgl}
                for a in s.legal_actions(agent.player_id):
                    s2 = s.clone(); s2.apply_action(a); walk(s2)
            else:
                for a in s.legal_actions():
...
def run_match(game, agent, opp_player, pure_strats, iss_list,
    ...
    for hand_num in range(1, num_hands + 1):
        ...
        # Violation check per paper Definition 7.1: expl(π_t) ≤ k_t.
        # Only meaningful when the agent actively deviated (k > 0 and chose
        # an exploitative action). When k ≤ 0 the agent plays the fallback
        # minimum-expl strategy and the invariant cannot fire.
        threshold = max(k_before, 0.0)
        safe_ok   = (threshold == 0.0) or (expl <= threshold + 1e-9)
        if not safe_ok:
            violations += 1
...
def run_test_suite(num_runs=NUM_RUNS, base_seed=None):
    ...
    agent_step_fn = make_agent_step_exact(game)
    ...
    w = 78
    print(f"\n{'='*w}")
    print(f"  RESULTS  ({num_runs} runs × {NUM_HANDS} hands each)  —  RWYWEAgent, player 0")
    print(f"  v* = {v_star:+.6f}")
    print(f"{'='*w}")
    print(f"  {'Opponent':<44} {'Mean pay':>9} {'± std':>7} "
          f"{'Above v*':>9} {'k_max':>7} {'Viol/run':>9}")
    print(f"  {'-'*44} {'-'*9} {'-'*7} {'-'*9} {'-'*7} {'-'*9}")
