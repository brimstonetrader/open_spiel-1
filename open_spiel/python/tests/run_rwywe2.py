...
def run_variant(warmup_hands, num_hands=NUM_HANDS, opp_seed=7, base_hand_seed=42):
    all_profits = np.zeros(num_hands)
    all_ks      = np.zeros(num_hands)

    # Build opponent once — MCCFR solve is expensive, same opp across seeds
...
if __name__ == "__main__":
    ...
    for warmup in WARMUPS:
        ...
        profits, ks = run_variant(warmup, num_hands=NUM_HANDS)
