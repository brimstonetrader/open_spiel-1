...
def with_one_hot_action_features(state_features, legal_actions,
  ...
  for action in legal_actions:
    action_features = F.one_hot(
        torch.tensor([action]), num_classes=num_distinct_actions).float()
    base = torch.cat([state_features, action_features], dim=1)
    scaled = alpha * base
    all_features = torch.cat([base, scaled], dim=1)
    
    additional_element = torch.tensor([[alpha]])
    all_features_with_extra = torch.cat([all_features, additional_element], dim=1)
    
    with_action_features.append(all_features_with_extra)
...
def sequence_features(state, num_distinct_actions, alpha):
  ...
  """
  base_features = with_one_hot_action_features(state.information_state_tensor(state.current_player()),
                                               state.legal_actions(),
                                               num_distinct_actions, alpha)
  
  num_sequences = base_features.shape[0]
  additional_feature = torch.full((num_sequences, 1), alpha)
  
  return torch.cat([base_features, additional_feature], dim=1)
...
def num_features(game):
  ...
  """
  return 16#(game.information_state_tensor_size() + game.num_distinct_actions() ) * 2
...
class _RcfrSolver(object):
  ...
  def __init__(self, game, models, alpha, truncate_negative=False):
  ...
  def _sequence_weights(self, player=None):
    ...
    weights = np.append(weights, self.alpha)
...
class RcfrSolver(_RcfrSolver):
  ...
  def __init__(self, game, models, alpha, bootstrap=None, truncate_negative=False):
  ...
  def evaluate_and_update_policy(self, train_fn, alpha):
    """Performs a single step of policy evaluation and policy improvement.
